  ...
  <div class="d-flex align-center">
    <v-menu v-model="menu" location="bottom" :close-on-content-click="false">
      <template v-slot:activator="{ props }">
        <v-btn v-bind="props" :ripple="false" class="v-custom-button px-2 text-gray-0 custom-datepicker">
          {{ dateType === 'monthMode' ? monthLabel : dayLabel }}
        </v-btn>
      </template>
      <v-card>
        <v-row class="ma-0 bg-datepicker">
          <v-sheet class="pa-3 d-flex flex-column align-self-start" color="datepicker">
            <v-radio-group
              v-model="radio"
            ...
            >
              <v-radio :value="item.value" color="fourth" class="custom-radio">
          ...
          <v-divider vertical color="gray-60" class="custom-divider"></v-divider>
          <div class="pa-3">
            <Datepicker
            ...
            >
              <template #menu-header>
                <div class="v-text-body-2 text-on-datepicker d-flex justify-space-between">
                  <span class="align-self-center"> {{ $t(dateHeader[0]) }}: </span>
                  <v-sheet
                  ...
                  <span class="align-self-center">{{ $t(dateHeader[1]) }}:</span>
                  <v-sheet
                    :width="dateType === 'monthMode' ? 170 : 190"
                    height="36"
                    class="bg-dateheader text-on-dateheader d-flex align-center px-2"
...
const defaultmMonth = initValue.value.length === 0 ? previousMonth(today, 6) : initValue.value
const month = ref(defaultmMonth)
const previousTwoDays = new Date(new Date().setDate(today.getDate() - 2))
const defaultDay =
  initValue.value.length === 0 ? previousMonth(previousTwoDays, 6) : initValue.value
...
const dp = ref()
let maxDate =
  max.value !== 'null' ? max.value : dateType.value === 'monthMode' ? today : previousTwoDays
let radio = ref('')
let datePeriod = ref(formatMonthToString(month.value))
let isMoreOneYear = ref<boolean | undefined>(false)
const dateHeader = ref(['general.periodSelect.from', 'general.periodSelect.to'])
let tempDate = ref([])
...
const close = () => {
  ...
  if (dateType.value === 'monthMode') {
    const temp = [
      { month: unChangeDate[0].getMonth(), year: unChangeDate[0].getFullYear() },
      { month: unChangeDate[1].getMonth(), year: unChangeDate[1].getFullYear() }
    ]
    ...
    : [
        ...
        },
        {
          ...
          day: [
            new Date(
              startDay.setFullYear(previousTwoDays.getFullYear()) &&
                startDay.setMonth(startMonthForQuarterToDate(previousTwoDays)) &&
                startDay.setDate(1)
            ),
            new Date(previousTwoDays)
          ],
          disabled:
            [0, 3, 6, 9].includes(today.getMonth()) || [0, 3, 6, 9].includes(endDay.getMonth())
              ? true
              : false
        },
        {
          ...
          disabled: today.getMonth() === 0 || endDay.getMonth() === 0 ? true : false
        },
        { text: 'general.periodSelect.custom', value: 'custom', month: [], disabled: false }
      ]
)
...
function changeMonthPeriod(data: any) {
  if (presetValue.value.length !== 0) {
    ...
    switch (data) {
      ...
      case 'lastYear':
        month.value = [
          { month: currentDate.getMonth() - 1, year: currentDate.getFullYear() - 1 },
          { month: currentDate.getMonth() - 1, year: currentDate.getFullYear() }
        ]
        day.value = [
      ...
      case 'quarterToDate':
        month.value = [
          { month: startMonthForQuarterToDate(currentDate), year: currentDate.getFullYear() },
          { month: currentDate.getMonth(), year: currentDate.getFullYear() }
        ]
        day.value = [
          new Date(startDay.setDate(1) && startDay.setMonth(startMonthForQuarterToDate(startDay))),
          new Date(endDay)
        ]
        break
      case 'yearToDate':
        ...
        day.value = [new Date(startDay.setDate(1) && startDay.setMonth(0)), new Date(endDay)]
        break
      case 'custom':
        break
    }
...
function adjustPeriod(data: any) {
  ...
  const presetOptionsMonthString = presetOptions.value.map((option) => JSON.stringify(option.month))
...
function formatMonthToString(dates: any) {
  ...
  if (dates.length > 1) {
    ...
    if (dates[1].year === currentYear && dates[1].month === currentMonth) {
      ...
      endDateString = `${endDate.getFullYear()}${String(endDate.getMonth() + 1).padStart(
        2,
        '0'
...
function isDateString(dateString: any) {
  return typeof dateString === 'string' && dateString.length === 8 && !isNaN(Number(dateString))
}
...
watch(
  ...
  (newVal) => {
    if (dateType.value === 'monthMode') {
      month.value = newVal
      startDate.value = new Date(month.value[0].year - 1, month.value[0].month + 1, 1)
      datePeriod.value = formatMonthToString(month.value)
    } else {
      if (isDateString(newVal[0])) {
        day.value = newVal.map((date) => formatDates(date))
        startDate.value = new Date(day.value[0].getFullYear(), day.value[0].getMonth(), 1)
        const changeDate = formatDayToString(newVal.map((date) => formatDates(date)))
...
.custom-datepicker {
  background: rgb(var(--v-theme-primary), 0.6) !important;
}
.apply-button {
  background-color: rgba(var(--v-theme-fourth));
  color: rgba(var(--v-theme-gray-100));
  padding: 8px 24px;
  margin-left: 12px;
  border-radius: 4px;
  font-size: 12px;
...
.cancel-button {
  color: rgba(var(--v-theme-fourth));
  padding: 8px 20px;
  border: 1px solid rgba(var(--v-theme-fourth));
  border-radius: 4px;
  font-size: 12px;
}
.custom-font-size {
