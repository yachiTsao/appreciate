...
const { period, chartData, singleSeries, stack, legendFontSize } =
  toRefs(props);
    ...
    for (let i = 0; i < params.length; i++) {
      if (params[i].value) {
        ...
        result += `<div style="
                ...
                font-size: 12px;
                font-weight: 500;
              "><div> ${params[i].marker} ${params[i].seriesName} </div> <div> $${params[
                i
              ].value
                .toFixed(2)
...
watch(
  () => props.theme,
  (newVal) => {
    barOption.value.xAxis[0].axisLabel.color =
      newVal === "darkTheme" ? "#FFFFFF" : "#556675";
    barOption.value.yAxis[0].axisLabel.color =
      newVal === "darkTheme" ? "#FFFFFF" : "#556675";
  },
  { immediate: true },
);
